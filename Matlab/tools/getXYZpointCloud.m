...
% queryPoint
camPosition1 = [143419.7536689211; 6394346.343762503; 34.26819042532588];
camPosition2 = [143423.2832925858 6394330.28638955 35.987895485362978];


%% find the closest neighbours 
% set the query point
queryPoint = [143425.6844188932 6394339.576775855 35.996698161994570];
queryPoint2 = [143425.6844188932 6394350.576775855 35.996698161994570];
queryPoint3 = [143455.8528600886 6394421.110296419 36.6508758021543];
queryPointWith37Corr = [143425.236678384 6394341.264853545 35.02142530144192];
[entranceIndices, entranceDists] = findNearestNeighbors(gnistangtunnelnPC, queryPointWith37Corr, 6000000);
% [entranceIndices2, entranceDists2] = findNearestNeighbors(gnistangtunnelnPC, queryPoint3, 2500000);

entranceXYZ = gnistangtunnelnXYZ(entranceIndices, 1:3);

%% show the closest neighbours
figure;
text  (143430.307000000, 6394362.90800000, 39.621, sprintf('Center NNeighbor')); hold on;
pcshow (gnistangtunnelnXYZ(entranceIndices, 1:3)); hold on;
% figure;
% pcshow (gnistangtunnelnXYZ(entranceIndices2, 1:3)); hold on;
...
projectedcameracenter(1:3,:) = queryPoint;
